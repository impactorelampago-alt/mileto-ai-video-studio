...
test('minimumAppVersion exige atualização sem bloquear a própria 1.4.32', () => {
    const worker = loadClientModule('opsVideoWorkerState.ts');
    assert.equal(worker.opsWorkerSupportsMinimumVersion('1.4.31', '1.4.32'), true);
    assert.equal(worker.opsWorkerSupportsMinimumVersion('1.4.32', '1.4.32'), true);
    assert.equal(worker.opsWorkerSupportsMinimumVersion('1.4.33', '1.4.32'), false);
    assert.equal(worker.opsWorkerSupportsMinimumVersion('v2.0.0', '1.4.32'), false);
});
