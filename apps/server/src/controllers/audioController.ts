...
export const mixAudio = async (req: Request, res: Response) => {
    try {
        ...
        const narrationSpeed = audioConfig?.narration?.speed ?? 1.0;
        const musicVol = audioConfig?.background?.enabled ? (audioConfig.background.volume ?? 0.2) : 0;
        const musicSpeed = audioConfig?.background?.speed ?? 1.0;
        ...
        if (inputCount === 1) {
            ...
            const singleSpeed = narrationPath && narrationVol > 0 ? narrationSpeed : musicSpeed;

            await new Promise<void>((resolve, reject) => {
                const singleFilter =
                    singleSpeed !== 1.0 ? `atempo=${singleSpeed},volume=${singleVol}` : `volume=${singleVol}`;

                command
                    .audioFilters(singleFilter)
                    .save(outputPath)
                    .on('end', () => resolve())
        ...
        await new Promise<void>((resolve, reject) => {
            // Se o speed for != 1.0, aplicamos atempo.
            const narFilter =
                narrationSpeed !== 1.0 ? `atempo=${narrationSpeed},volume=${narrationVol}` : `volume=${narrationVol}`;

            const musFilter = musicSpeed !== 1.0 ? `atempo=${musicSpeed},volume=${musicVol}` : `volume=${musicVol}`;

            command
                .complexFilter([
                    `[0:a]${narFilter}[a0]`,
                    // We loop the background music infinitely (-1) so it never ends early,
                    // then amix shortest will cut the entire mix when narration ends
                    `[1:a]aloop=loop=-1:size=2e9,${musFilter}[a1]`,
                    `[a0][a1]amix=inputs=2:duration=shortest:dropout_transition=2:normalize=0[aout]`,
                ])
                .map('[aout]')
                .save(outputPath)
                .on('end', () => resolve())
        ...
        const errMsg = error instanceof Error ? error.message : 'Erro ao mixar áudios';
