...
export const generateMoodRewrite = async (req: Request, res: Response) => {
    const { text, mood, openaiKey, geminiKey } = req.body;

    if (!text || !mood) {
        return res.status(400).json({ ok: false, message: 'Missing text or mood' });
    }
    ...
    const systemPrompt = `Você é um especialista em reescrita semântica para síntese de voz (TTS).
Sua missão é reescrever o texto fornecido pelo usuário para que, quando lido por uma Inteligência Artificial de voz neutra, soe com o humor solicitado: "${mood}".

Regras estritas:
1. MANTENHA O SENTIDO ORIGINAL do texto (mesmos fatos, mesma mensagem principal).
2. USE APENAS ferramentas de texto para forçar o sentimento: pontuação exagerada (!!!, ...), interjeições (Ah!, Hahaha!, Uau, Nossa), repetições leves ou hesitações.
3. NÃO MUDE A PERSONA. Se é uma propaganda de carro, continua sendo, mas no humor pedido.
4. Responda APENAS com o texto reescrito, sem aspas, sem introduções e sem JSON. Apenas o texto bruto perfeito pronto para ir pro sintetizador.`;

    try {
        let rewrittenText = text;

        if (openaiKey) {
            console.log('[AI] Reescrevendo humor com OpenAI...');
            const openai = new OpenAI({ apiKey: openaiKey });
            const response = await openai.chat.completions.create({
                model: 'gpt-4o-mini',
                messages: [
            ...
            rewrittenText = response.choices[0].message.content?.trim() || text;
        } else if (geminiKey) {
            console.log('[AI] Reescrevendo humor com Gemini...');
            const response = await axios.post(
                `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${geminiKey}`,
                {
                    contents: [
            ...
            rewrittenText = response.data.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || text;
        }

        res.json({ ok: true, text: rewrittenText });
    } catch (error: any) {
        console.error('[AI] Erro ao reescrever humor:', error.response?.data || error.message);
        res.status(500).json({ ok: false, message: 'Falha ao reescrever texto com IA.' });
    }
};
