    ...
    const play = useCallback(() => {
        ...
        timeline.tracks.forEach((track) => {
            ...
            track.clips.forEach((clip) => {
                ...
                // If the track is sped up (e.g. 2x), the audio source plays faster,
                // so the actual duration it takes in real time is duration / speed.
                // However, playDuration is passed to source.start() as the duration *in the source file* to play.
                // Wait, WebAudio API source.start(when, offset, duration) parameter 'duration' is the *duration of the buffer data to be read*, NOT the logical output time.
                // Actually, MDN says: duration is the duration of the portion to be played, in seconds.
                // Let's keep playDuration as the source buffer length we want to consume.

                if (playDuration <= 0) return;

                const source = ctx.createBufferSource();
                source.buffer = buffer;
                source.playbackRate.value = speed; // Apply the UI speed here!
