...
import { ArrowLeft, LogOut, Wallet, Users, Activity, Loader2, Trash2, UserPlus } from 'lucide-react';
...
export const Account = () => {
    ...
    const { user, balance, logout, refreshMe } = useAuth();
    ...
    return (
        <div className="min-h-screen w-full bg-background text-foreground">
            {/* Topbar */}
            <div className="sticky top-0 z-30 border-b border-border/50 bg-background/80 backdrop-blur-sm px-4 py-2.5 flex items-center justify-between">
                <button
                    onClick={() => navigate('/')}
                    className="flex items-center gap-2 px-2 py-1 rounded-xl hover:bg-white/5 transition-all group"
                >
                    <ArrowLeft className="w-4 h-4 text-foreground/70" />
                    <img src={logoImg} alt="Mileto AI" className="w-6 h-6 object-contain" />
                    <span className="text-[11px] font-black uppercase tracking-widest text-foreground/80">Voltar</span>
                </button>
                <button
                    onClick={() => void logout()}
