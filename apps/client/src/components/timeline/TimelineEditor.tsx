...
export const TimelineEditor: React.FC<TimelineEditorProps> = ({ isOpen, onClose }) => {
    ...
    const handleAutoZoom = React.useCallback(() => {
        ...
        const maxClipEnd = timeline.tracks.reduce((max, track) => {
            ...
            const trackEnd = track.clips.reduce((tMax, clip) => {
                ...
                const clipDur = ((duration || 30) - clip.inSec) / speed;
                ...
                const newTracks = prev.tracks.map((track) => {
                    ...
                    return {
                        ...
                        clips: track.clips.map((clip) => {
                            ...
                            const newClip = { ...clip };
                            const sourceChange = deltaSec * speed;

                            if (dragState.type === 'move') {
                                newClip.startSec = Math.max(0, dragState.initialStart + deltaSec);
                            } else if (dragState.type === 'trim-start') {
                                newClip.startSec = Math.max(0, dragState.initialStart + deltaSec);
                                newClip.inSec = Math.max(0, dragState.initialIn + sourceChange);
                            } else if (dragState.type === 'trim-end') {
                                newClip.outSec = Math.max(newClip.inSec + 0.1, dragState.initialOut + sourceChange);
                            }
    ...
    const handleSpeedChange = React.useCallback((trackId: string, newSpeed: number) => {
        setTimeline((prev) => {
            if (!prev) return null;
            const newTracks = prev.tracks.map((t) => (t.id === trackId ? { ...t, speed: newSpeed } : t));

            // Recalculate duration incorporating the speed of the track to get visual max
            const maxEnd = newTracks.reduce((max, track) => {
                const speed = track.speed ?? 1.0;
                return Math.max(
                    max,
                    track.clips.reduce((tm, c) => Math.max(tm, c.startSec + ((c.outSec || 30) - c.inSec) / speed), 0)
                );
            }, 0);
