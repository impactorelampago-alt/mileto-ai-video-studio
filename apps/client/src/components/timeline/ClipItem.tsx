...
export const ClipItem: React.FC<ClipItemProps> = ({ clip, zoom, audioBuffer, isSelected, onSelect, onMouseDown }) => {
    ...
    const duration = (resolvedDuration && resolvedDuration > 0 ? resolvedDuration : 30) - clip.inSec;

    const width = Math.max(10, timeToX(duration, zoom));
    ...
    React.useEffect(() => {
        ...
        const endSample = Math.floor((clip.inSec + duration) * sampleRate);
    ...
    }, [audioBuffer, width, clip.inSec, duration, zoom]); // Re-render if zoom/trim changes
