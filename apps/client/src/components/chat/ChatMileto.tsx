...
export const ChatMileto: React.FC = () => {
    ...
    // ─── Welcome Balloon: notification sound + timing ─────────────────────────
    const playNotificationSound = () => {
        try {
            ...
            g1.gain.linearRampToValueAtTime(0.14, ctx.currentTime + 0.02);
            ...
            connect(osc2, g2);
            osc2.type = 'sine';
            osc2.frequency.setValueAtTime(1108, ctx.currentTime + 0.12);
            g2.gain.setValueAtTime(0, ctx.currentTime + 0.12);
            g2.gain.linearRampToValueAtTime(0.1, ctx.currentTime + 0.14);
    ...
    useEffect(() => {
        ...
        const t2 = setTimeout(() => setBalloonExiting(true), 3800);
        const t3 = setTimeout(() => {
    ...
    if (!isOpen) {
        return (
            <>
                {/* ── Balloon keyframe styles ─────────────────────────────── */}
                <style>{`
                    @keyframes balloonIn {
                        0%   { opacity: 0; transform: scale(0.4) translateX(24px); }
                        65%  { transform: scale(1.06) translateX(-4px); }
                        100% { opacity: 1; transform: scale(1) translateX(0); }
                    }
                    @keyframes balloonOut {
                        0%   { opacity: 1; transform: scale(1) translateX(0); }
                        100% { opacity: 0; transform: scale(0.85) translateX(18px); }
                    }
                `}</style>
                ...
                {showBalloon && (
                    <div
                        ...
                        style={{
                            left: Math.max(12, btnPos.x - 270),
                            top: btnPos.y + 2,
                            zIndex: 9999,
                            animation: balloonExiting
                                ? 'balloonOut 0.35s cubic-bezier(0.4,0,0.6,1) forwards'
                                : 'balloonIn 0.5s cubic-bezier(0.175,0.885,0.32,1.275) forwards',
                            transformOrigin: 'right center',
                        }}
                    >
                        {/* Bubble body */}
                        <div
                            className="relative rounded-2xl px-4 py-3 max-w-[230px]"
                            style={{
                                background: 'linear-gradient(135deg,rgba(0,12,6,0.97) 0%,rgba(0,22,10,0.97) 100%)',
                                border: '1px solid rgba(0,230,118,0.38)',
                                boxShadow: '0 0 28px rgba(0,230,118,0.18), 0 8px 40px rgba(0,0,0,0.75)',
                                backdropFilter: 'blur(20px)',
                            }}
                        >
                            <p
                                className="text-sm font-semibold leading-relaxed"
                                style={{ color: 'rgba(242,255,247,0.95)' }}
                            >
                                Oiii meu bem, que narração quer que eu crie hoje?{' '}
                                <span style={{ color: '#00e676' }}>💚</span>
                            </p>

                            {/* Tail border (green outline) */}
                            <div
                                className="absolute"
                                style={{
                                    right: -10,
                                    ...
                                    borderLeft: '11px solid rgba(0,230,118,0.38)',
                                }}
                            />
                            {/* Tail fill (dark bg) */}
                            <div
                                ...
                                style={{
                                    ...
                                    borderLeft: '9px solid rgba(0,22,10,0.97)',
                                }}
                            />

                            {/* Subtle brand-accent inner glow pulse */}
                            <div
                                className="absolute inset-0 rounded-2xl pointer-events-none"
                                style={{
                                    background:
                                        'radial-gradient(ellipse at 20% 50%, rgba(0,230,118,0.06) 0%, transparent 70%)',
                                }}
                            />
                        </div>
                    </div>
