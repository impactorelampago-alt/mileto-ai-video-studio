...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({ src, className }) => {
    ...
    return (
        <div className={cn('bg-slate-800 rounded-lg p-3 flex items-center gap-3', className)}>
            ...
            <button
                onClick={togglePlay}
                className="w-8 h-8 rounded-full bg-primary hover:bg-primary-hover text-slate-950 flex items-center justify-center transition-colors shrink-0"
            >
                {isPlaying ? (
            ...
            <button
                ...
                title="Baixar Áudio"
                className="text-slate-400 hover:text-foreground transition-colors ml-1"
            >
